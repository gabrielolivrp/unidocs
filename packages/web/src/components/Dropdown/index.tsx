...
export const DropdownItem = ({ text, onClick, icon: Icon }: DropdownItemProps) => (
  <Menu.Item>
    {({ active }) => (
      <button onClick={onClick} className={`group flex rounded-md items-center space-x-2 w-full px-2 py-2 text-md ${active ? 'text-primary dark:text-primary' : 'text-gray-700 dark:text-gray-50'}`}>
        {Icon && <Icon />}
